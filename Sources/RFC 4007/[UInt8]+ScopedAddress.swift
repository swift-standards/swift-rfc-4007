...
extension [UInt8] {
    /// Creates ASCII bytes from RFC_4007.IPv6.ScopedAddress
    ///
    /// ## Category Theory
    ///
    /// Natural transformation: RFC_4007.IPv6.ScopedAddress → [UInt8]
    /// ```
    /// ScopedAddress → [UInt8] (ASCII) → String (UTF-8)
    /// ```
    ///
    ...
    /// let bytes = [UInt8](scoped)
    /// // bytes == "fe80::1%eth0" (as ASCII bytes)
    /// ```
    public init(_ value: RFC_4007.IPv6.ScopedAddress) {
        // Get canonical RFC 5952 representation of the address
        self = [UInt8](value.address)

        if let zone = value.zone {
            // RFC 4007 Section 11.7: Format is <address>%<zone_id>
            self.append(UInt8.ascii.percentSign)
            self.append(contentsOf: zone.utf8)
        }
    }
}
